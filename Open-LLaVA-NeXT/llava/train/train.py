...
class ModelArguments:
    ...
    image_processor_name_or_path: Optional[str] = field(default=None)
    ...
    num_steps: Optional[int] = field(default=None)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    gradient_checkpointing_kwargs: Optional[Dict[str, bool]] = field(default_factory = lambda: ({"use_reentrant": True}))
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            if model_args.epsilon is not None:
                assert model_args.step_size is not None and model_args.num_steps is not None, "adv training needs epsilon, step_size, and num_steps!"
                model.adv=dict(epsilon=model_args.epsilon, step_size=model_args.step_size, num_steps=model_args.num_steps)
    else:
    ...
    # kludge for transformer incompatability
    # https://github.com/haotian-liu/LLaVA/issues/1144
    if trainer.model.generation_config.temperature or trainer.model.generation_config.top_p:
        print(f"setting model.generation_config.do_sample = True because of temperature {trainer.model.generation_config.temperature} and top_p {trainer.model.generation_config.top_p}")
    ...
    # no need to delay load. vision_tower weight is saved as well
    if not model_args.tune_mm_mlp_adapter and model_args.pretrain_vision_tower is not None:
        trainer.model.config.mm_vision_tower_delay_load = False
        # if model_args.pretrain_vision_tower is not None:
        #     trainer.model.config.pretrain_vision_tower = model_args.pretrain_vision_tower
        if model_args.image_processor_name_or_path is not None:
            trainer.model.config.image_processor_name_or_path = model_args.image_processor_name_or_path

        if training_args.lora_enable and not training_args.unfreeze_mm_vision_tower:
            trainer.model.config.pretrain_vision_tower = model_args.pretrain_vision_tower
            trainer.model.config.mm_vision_tower_delay_load = True
