...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None, output_hidden_states: Optional[bool] = False):
        if not self.batch_first:
            x = x.transpose(0, 1).contiguous()    # NLD -> LND
        ...
        if not self.batch_first:
            x = x.transpose(0, 1)    # LND -> NLD
            if output_hidden_states:
                encoder_states = [encoder_state.transpose(0, 1) for encoder_state in encoder_states]
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, output_hidden_states: Optional[bool] = False):
        x = self._embeds(x)
        if output_hidden_states:
            x, encoder_states = self.transformer(x, output_hidden_states=output_hidden_states)
        else:
        ...
        if output_hidden_states:
            assert not self.output_tokens
            return pooled, encoder_states
