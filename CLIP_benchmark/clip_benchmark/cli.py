...
def get_parser_args():
    ...
    parser_eval.add_argument(
        ...
        help='Override default image size value of dataset')
    parser_eval.add_argument(
    ...
    parser_eval.add_argument(
        ...
        help='Interpolation mode for resizing during inference. only supported for open_clip_big_vision. accepted value: bicubic, bilinear.')
    parser_eval.add_argument('--square-resize-only', default=False, action="store_true", help="square resize only.")
    #################
...
def run(args):
    ...
    if args.skip_load:
        ...
        model, transform, tokenizer = load_clip(
            ...
            square_resize_only=args.square_resize_only,
        )
        # FIXME when evaluating robust clip, should include these code
        ...
        #     resize = Resize(size=224 if args.image_size is None else args.image_size, interpolation=transforms.InterpolationMode.BICUBIC, max_size=None, antialias=None)
        # else:
